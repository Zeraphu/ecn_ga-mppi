...
class MPPI_GA():
    ...
    def compute_costs(self, ref_path, paths, inputs):
        ...
        total_costs = np.sum([euc_costs, ctr_costs, advance_costs], axis=0)
    ...
    def crossover_mutate(self, parents, mutation_rate=0.1):
        """
        Performs crossover and mutation to generate new children from parents.
        ...
        Returns:
            children (np.ndarray): New population of shape (num_children, N, 2).
        """
        ...
        for i in range(num_parents):
            ...
            crossover_point = np.random.randint(1, self.horizon - 1)
    ...
    def run(self, 
        ...
        self.population, paths = utils.generate_population(self.horizon, self.num_paths, 
                                    ...
                                    current_state[2],
                                    self.dynamics)
        print(self.population.shape, "population shape")
        start = time.time()
        while True:
            generation += 1
            # 2. Compute costs
            ...
            if generation > 1000 or np.min(self.costs) < 0.15 or time.time() - start > 1:
                best_U = self.population[np.argmin(self.costs)]
...
if __name__ == "__main__":
    ...
    mppi_ga = MPPI_GA(horizon, pop_size, motionModel.turtleModel())
